...
def Stack():
    ...
    return []
    #el extend concatena dos listas, entonces añade la lista 2 por detrás de la lista1, y el pop() saca del final --> Esto es una pila
...
class FIFOQueue(Queue):
    ...
    # Modificamos el pop() porque nos interesa extraer desde el principio.
    def pop(self):
        e = self.A[self.start]
        self.start += 1
...
class path_Cost(Queue):
    "Return a list with lowest path_cost"
    ...
    # en el graph_search despues del extend, la siguiente instrucción es pop --> Me interesa que cuando extraiga este
    # reordenado por el menor coste --> de ahí que lo hagamos en el método extend
...
class best_heuristica(Queue):
    ...
    # hallar la suma de la heuristica y el path_cost y a continuación reordenarlo
    def extend(self, items):
        self.A.extend(items)
